...
import com.sdust.mall.realtime.bean.SkuInfo
import com.sdust.mall.realtime.util.{MyKafkaUtil, OffsetManagerUitl, PhoenixUtil}
  ...
  def main(args: Array[String]): Unit = {
    ...
    val skuInfoDstream: DStream[SkuInfo] = objectDstream.transform {
      rdd =>{
        if(rdd.count()>0){
          //tm_name
          val tmSql = "select id ,tm_name from mall2021_base_trademark"
          ...
          //category3
          val category3Sql = "select id ,name from mall2021_base_category3"
          //driver 周期性执行
          ...
          => (jsonObj.getString("ID"), jsonObj)).toMap
          // spu
          val spuSql = "select id ,spu_name from mall2021_spu_info" // spu
